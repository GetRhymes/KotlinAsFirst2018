...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun distance(other: Circle): Double {
        ...
        val distanceRadius = radius + other.radius
        return if (distanceCenter <= distanceRadius) 0.0
        else distanceCenter - distanceRadius
    }
...
fun diameter(vararg points: Point): Segment {
    ...
    for (i in 0 until points.size)
        for (el in i + 1 until points.size)
            if (points[i].distance(points[el]) > begin.distance(end)) {
                end = points[el]
                begin = points[i]
            }
